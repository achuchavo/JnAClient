...
uses
  SysUtils, Windows,IdCoder, IdCoderMIME,IdGlobal{, TntLXUtils};
...
begin
  if (not CryptAcquireContext(Result, nil, nil, ProviderType, 0)) then //Вылетает Access Violation !! ЗДЕСЬ
  ...
  if (Value = '') then
    ...
    try
      ...
      try
        ...
        Buffer := IndyTextEncoding_UTF8.GetBytes(Value + '        ');
  ...
  if Value = '' then
    ...
    try
      ...
      try
        ...
        Result := IndyTextEncoding_UTF8.GetString(Buffer, 0, lul_datalen);
